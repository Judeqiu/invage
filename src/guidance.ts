...
const portfolio = (): string =>
  [
    '*Portfolio (holdings)*',
    '',
    'I store **equities, funds (ETF / 基金 / MMF), and options** on your user profile.',
    '',
    '*Equities*',
    ...
    '• Examples: PHILLIPUSDMMF, FULLERTONSGDLIQ, CN fund codes — **never** import as equity (breaks dashboard)',
    '• If NAV unknown at import, set mark = avg_price; update mark later when you have NAV',
    '',
    '*Options (calls / puts)*',
    '• US-style: **1 contract controls 100 shares** (`multiplier=100`) — used for **assignment size only**',
    '• `avg_price` / `mark` = **total premium dollars per contract** (e.g. sold put for $265 total → avg_price=265)',
    '• Do **not** multiply premium by 100 again; total premium = avg_price × contracts',
    '• `units` = number of **contracts**',
    '• Required: right (call|put), side (long|short), strike, expiry (YYYY-MM-DD), multiplier, underlying, settlement (physical|cash)',
    '• Open short is not "triggered" until assigned: MTM = ±mark (option premium), not strike loss',
    '• Contingent obligation (if assigned) is separate: short put cash = strike × multiplier × contracts',
    '• Live marks: **public listed** underlyings → Yahoo options chain (auto); **private/OTC** → stored `mark`',
    '• `quote_source=manual` forces stored mark; `quote_source=yahoo` requires a Yahoo chain match (fails if missing)',
    '• Private underlyings OK (e.g. SPACEX) — auto falls back to stored mark',
    '• Example: "Short put SpaceX, 1 ct, strike $90, premium $265 total, expiry 2026-08-07, physical"',
    '  → key `SPACEX-P-90-20260807-S`, premium $265, MTM −$265 at entry, contingent if assigned $9,000',
    '• Short calls: contingent share delivery if assigned (contracts × multiplier)',
    '',
    '*Other*',
    '• "Remove TSLA" / "Show my portfolio" / "Clear portfolio" (I will confirm first)',
    '• Category tip: use fund labels like `SL Technology S1`, `Private / Secondary`, …',
    '',
    '*What I do*',
    '• Use portfolio tools bound to *your* Slack/Telegram id (never invent another user\'s id)',
    '• Options never hit Yahoo under the contract key; equities still use live quotes',
    '',
    '*Next — analysis stack*',
    '• `/guidance analysis` — 3-axis P/L vs Street targets (equities) + options MTM block',
    '• `/guidance value` — advanced undervalued / trap screen on equity holdings',
    '• `/guidance reports` — save HTML report / snapshot / dashboard',
  ].join('\n');
