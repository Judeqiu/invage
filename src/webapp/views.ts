...
<body>
  <div class="card">
    <div class="logo">
      <svg viewBox="0 0 24 24" fill="none" stroke="#3fb950" stroke-width="2"><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg>
      <h1>InvesterDrive</h1>
    </div>
    <p class="subtitle">Investment report portal</p>
    ${error ? `<div class="error">${escapeHtml(error)}</div>` : ''}
    <form method="POST" action="/login">
      ${returnUrl ? `<input type="hidden" name="return" value="${escapeHtml(returnUrl)}">` : ''}
      <label for="username">Username <span style="color:#8b949e;font-size:0.75rem">( admins only, leave blank for drive token )</span></label>
    ...
    <p class="hint">Get your drive token from the Invester agent.</p>
  </div>
</body>
</html>`;
}
...
export function drivePage(
  ...
  const fileList = files.length > 0
    ...
    : '<tr><td colspan="4" class="empty">No files yet. Ask the Invester agent to generate a report.</td></tr>';
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>InvesterDrive — ${escapeHtml(targetSlug)}</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
