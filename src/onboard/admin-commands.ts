/**
 * Admin commands for the onboard flow (Slack):
...
function formatRow(t: OnboardToken): string {
  ...
  if (t.status === 'used') {
    return `${base}\n    used ${t.used_at} → \`${t.slug ?? '?'}\` by slack ${t.used_by_slack}`;
  }
...
export function handleOnboardCommand(ctx: CommandContext): string {
  ...
  if (sub === 'reject') {
    ...
    markRejected(token, ctx.slackUserId, reason);
