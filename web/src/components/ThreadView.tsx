...
export function ThreadView({ messages, viewerSlug, now }: ThreadViewProps) {
    ...
    return (
      <div className="flex flex-1 flex-col items-center justify-center px-6 py-12 text-center text-slate-500">
        ...
        <p className="max-w-md text-sm">
          Ask me about your portfolio — sector concentration, holdings,
          rebalancing, or anything else. Reports render inline here.
        </p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-3xl flex-col gap-4">
        {messages.map((m) => (
          <MessageView key={m.id} message={m} viewerSlug={viewerSlug} now={now} />
        ))}
        <div ref={bottomRef} />
      </div>
    </div>
