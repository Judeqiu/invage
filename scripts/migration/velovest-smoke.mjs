...
if (url.pathname !== '/velovest_v4_rehearsal_20260916') throw new Error('Rehearsal DB required');
...
try {
 ...
 framework=await createFramework({database:db,defaultAgentId:'invage',agents:buildFrameworkAgentList('consultant'),startTaskScheduler:false,startWorkflowRuntime:false,startLifecycleMailScheduler:false});
 ...
 const root='/opt/velovest-v4-rehearsal/source-snapshot/users';
 let users=0;
 for(const file of readdirSync(root).filter(name=>name.endsWith('.yaml'))) {
